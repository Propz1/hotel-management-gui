<template>
    <div
      ...
      @click="clickEvent"
    >
      <span style="float: right; padding: 0px" @click="deleteEvent">✖</span
      ><span class="head">
        <!-- <span class="startTime time" style="font-size: 0.9em;">{{ startText }}</span
        >～<span class="endTime time" style="font-size: 0.9em;">{{ endText }}</span> -->
      </span>
      <span class="text" style="font-size: 0.68em; padding-left: 0.3em; padding-top: 0.39em;">{{ contentText }}</span>
      <div
        class="resizable-e"
        :draggable="'true'"
        @dragstart="editStart"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const setLeftPosition = () => {
        ...
        //state.styleObject.left = shiftLeft +(shiftCnt/5) + "px"; //
        state.styleObject.left = props.unitWidth * shiftCnt - props.borderWidth + "px";
      };
      /**
      ...
      const setWidth = () => {
        ...
        let width = props.unitWidth * widthCnt;
        state.styleObject.width = width - 2 + "px";
      };
      /**
